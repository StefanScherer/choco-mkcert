...
url="https://github.com/FiloSottile/mkcert/releases/download/v${version}/mkcert-v${version}-windows-amd64.exe"

checksum=$(curl -L "${url}" | shasum -a 256 - | cut -f 1 -d " ")

sed -i.bak "s/<version>.*<\/version>/<version>${version}<\/version>/" mkcert.nuspec
